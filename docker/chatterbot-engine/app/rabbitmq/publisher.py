...
class Publisher:
    ...
    def publishNotifyMessage(self, message,correlation_id):
        connection = None
        try:
            connection = self._create_connection()
            channel = connection.channel()
 
            channel.exchange_declare(exchange=os.environ['NOTIFY_EXCHANGE'],
                                         passive=True)
            channel.basic_publish(exchange=os.environ['NOTIFY_EXCHANGE'],
                                      routing_key="",
